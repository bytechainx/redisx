...
fn parse_host_port(addr: &str) -> RedisResult<(String, u16)> {
    let addr = addr.trim();
...
fn validate_seed(seed: &str) -> RedisResult<()> {
    let seed = seed.trim();
    if seed.is_empty() {
...
/// 逗号分隔节点列表 → 去空白、去空项。
fn split_nodes(raw: &str) -> Vec<String> {
    raw.split(',')
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(str::to_owned)
...
/// 解析部署模式（不区分大小写）。
fn parse_mode(value: &str) -> RedisResult<RedisMode> {
    match value.trim().to_ascii_lowercase().as_str() {
